// A 3D depth-stacked image carousel — design supplied directly by the user,
// adapted to TypeScript for this app (drag/wheel/keyboard navigation, GSAP-
// eased focus transitions, autoplay with hover/focus pause). Used for the
// company profile's photo gallery (see CompanyProfile in
// src/components/fava/views.tsx) in place of the previous plain image grid.
...
export function DepthCarousel({
  ...
  slideLabel = (current, total) => `${current} of ${total}`,
  ...
  const layout = useCallback((pos: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const blurCfg = cfg.blur as number;
      const blurPx =
        blurCfg > 0
          ? Math.min(blurCfg, (back / Math.max(1, cfg.visibleCards as number)) * blurCfg)
          : 0;
      ...
      if (ov) ov.style.opacity = clamp(back * (cfg.falloff as number) * 1.25, 0, 0.86).toFixed(3);
    }
  }, []);
  ...
  const tweenTo = useCallback(
    (target: number, animate: boolean) => {
      ...
      const dur = animate && !reducedRef.current ? (cfg.duration as number) / 1000 : 0;
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      const cfg = cfgRef.current;
      if ((cfg.count as number) < 2) return;
      e.preventDefault();
      tweenRef.current?.kill();
      const raw = Math.abs(e.deltaX) > Math.abs(e.deltaY) ? e.deltaX : e.deltaY;
      const delta = e.deltaMode === 1 ? raw * 24 : raw;
      const step = clamp(delta / ((cfg.cardWidth as number) * 0.9), -0.6, 0.6);
      posRef.current += step;
      layout(posRef.current);
  ...
  const onPointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (!drag) return;
      const cfg = cfgRef.current;
      const stepPx = Math.max((cfg.cardWidth as number) * 0.55 * scaleRef.current, 40);
  ...
  return (
    ...
    >
      ...
      {showIndicators && count > 1 && (
        <div className="depth-carousel__dots" role="tablist" aria-label={groupLabel}>
          {data.map((_, i) => (
            <button
              key={i}
              type="button"
