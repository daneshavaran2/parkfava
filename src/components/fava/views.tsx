...
function DirectionsPanel({
  ...
  useEffect(() => {
    ...
    if (
      ...
      qLng.ok &&
      qLng.value != null &&
      coordinatesMatch(lat, lng, qLat.value, qLng.value)
    ) {
      setHighlighted(true);
      window.setTimeout(
  ...
  async function copyLink() {
    try {
      const url = buildCompanyLocationUrl(
  ...
  return (
    <div
      ref={panelRef}
      className={"panel directions-panel" + (highlighted ? " directions-panel-highlight" : "")}
      data-testid="directions-panel"
    >
      <h3>
        <Icon name="pin" size={18} className="pi" /> {t("company.directions")}
      </h3>
      {highlighted && (
        <div role="status" className="location-highlight-note">
          {t("company.location_link_opened")}
        </div>
      )}
...
export function Home() {
  ...
  const { t } = useTranslation();
  function askAI(q) {
    navigate({ to: "/exhibition", search: { q } as any });
  }
  return (
        ...
        <section className="hero">
          <div className="hero-grid">
            <div>
              ...
              <h1 className="h1">
                {t("home.title_a")} <span className="grad">{t("home.title_grad")}</span>{" "}
                {t("home.title_b")}
              </h1>
...
function HomeFallback() {
  ...
  return (
    <div className="view">
      <div className="shell">
        <p className="lead" style={{ padding: 40 }}>
          {t("common.loading")}
        </p>
      </div>
    </div>
...
function StatNum({ s }) {
  ...
  return (
    <div
      ref={ref}
      className="sb-item"
      style={{ "--accent": colorVar(s.c), "--accent-glow": glowVar(s.c) }}
    >
...
function Portal({ p }) {
  ...
  return (
    <Link
      ...
      style={{ "--pc": colorVar(p.c), textDecoration: "none", color: "inherit" }}
      {...tilt}
    >
      <span className="glow" />
      <div className="pico">
...
function useMergedCompanies(staticCompanies) {
  ...
  return useMemo(() => {
    ...
    for (const cc of cloudCompanies) {
      ...
      const base = map.get(cc.company_id) || {
        id: cc.company_id,
        tags: [],
        products: [],
        contact: {},
        color: "blue",
        initials: (cc.name || cc.company_id).slice(0, 2),
      };
      ...
      <div className="shell">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div className="nav-search" style={{ minWidth: 260 }}>
              ...
              <input
                ...
                placeholder={t("exhibition.search_placeholder")}
                aria-label={t("common.search")}
              />
            </div>
          </div>
        ...
        {list.length === 0 ? (
          <div className="empty">
            ...
            <p style={{ marginTop: 10 }}>{t("exhibition.empty")}</p>
          </div>
        ) : (
...
export function CompanyCard({ c }) {
  ...
  return (
    <Link
      to="/company/$id"
      params={{ id: c.id }}
      className="co-card tilt"
      style={{ "--cc": colorVar(c.color), textDecoration: "none", color: "inherit" }}
      {...tilt}
    >
      <div className="co-top">
        <div
          ...
          style={{ overflow: "hidden", background: logoSrc ? "#fff" : undefined }}
        >
          {logoSrc ? (
            <img
              src={logoSrc}
      ...
      <div className="co-meta">
        <div className="m">
          ...
          <span>{t("company.full_time")}</span>
        </div>
        <div className="m">
          <b className="num">{c.headcount_part_time != null ? toFa(c.headcount_part_time) : "—"}</b>
...
} from "@/components/admin/AttachmentPreview";
...
function AssetLink({ path, label }) {
  ...
  if (!src) return null;
  return (
    <a className="btn btn-ghost" href={src} target="_blank" rel="noopener" download>
      <Icon name="box" size={16} /> {label}
    </a>
  );
}
...
function DocumentsPanel({ atts }: { atts: CompanyAttachment[] }) {
  ...
  return (
    <div className="panel">
      <h3>
        <Icon name="box" size={18} className="pi" /> {t("company.documents")}
      </h3>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {visible.map((a) => (
          <AttachmentDocRow key={a.id} att={a} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <>
      <figure style={{ margin: 0, cursor: "pointer" }} onClick={() => url && setOpen(true)}>
        ...
        {att.title && (
          <figcaption style={{ marginTop: 6, fontSize: 12, color: "var(--ink-soft)" }}>
            {att.title}
          </figcaption>
        )}
      </figure>
      {open && url && (
        <AttachmentPreviewDialog att={att} url={url} onClose={() => setOpen(false)} />
      )}
    </>
  );
}
...
function AttachmentGallery({ atts }: { atts: CompanyAttachment[] }) {
  ...
  return (
    <div className="panel">
      <h3>
        <Icon name="grid" size={18} className="pi" /> {t("company.gallery")}
      </h3>
      <div
        style={{
          display: "grid",
    ...
    <div className="view" style={{ "--cc": cc }}>
      <div className="shell">
        ...
        <div className="profile-hero">
          <div
            className="ph-logo"
            style={{ "--cc": cc, background: cc_data?.logo_url ? "#fff" : undefined }}
          >
            {cc_data?.logo_url ? (
              <AssetImg
                path={cc_data.logo_url}
                alt={pickName(c, i18n.language)}
                style={{
                  width: "100%",
                  height: "100%",
              ...
              <image-slot
                ...
                shape="rounded"
                radius="20"
                placeholder={t("company.add_logo")}
              />
            )}
          </div>

          <div style={{ flex: 1, minWidth: 240 }}>
            <span className="eyebrow" style={{ color: glowVar(c.color) }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            {c.contact.email && (
              <a className="btn btn-ghost" href={"mailto:" + c.contact.email}>
                <Icon name="mail" size={17} /> {t("company.contact")}
              </a>
            )}
            {cc_data?.catalog_url && (
              <AssetLink path={cc_data.catalog_url} label={t("company.download_catalog")} />
            )}
          </div>
        </div>
        ...
        {cc_data?.video_url && (
          <div className="panel" style={{ marginTop: 16, padding: 16 }}>
            <h3 style={{ marginBottom: 10 }}>
              <Icon name="spark" size={18} className="pi" /> {t("company.intro_video")}
            </h3>
            <AssetVideo
              path={cc_data.video_url}
        ...
        <div className="profile-grid">
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--gap)" }}>
            <div className="panel">
              <h3>
                <Icon name="chart" size={18} className="pi" /> {t("company.stats")}
              </h3>
              <div className="kpi-row">
                <div className="kpi">
                  {(() => {
                    ...
                    const total =
                      ft != null || pt != null
                        ? (ft ?? 0) + (pt ?? 0)
                        : (cc_data?.headcount ?? c.workers ?? null);
                    return <b className="num">{total != null ? toFa(total) : "—"}</b>;
                  })()}
                  <span>{t("company.workforce")}</span>
                  {(cc_data?.headcount_full_time != null ||
                    cc_data?.headcount_part_time != null) && (
                    <small
                      style={{
                        fontSize: 11,
                        color: "var(--ink-soft)",
                    ...
                    >
                      {cc_data?.headcount_full_time != null && (
                        <>
                          {toFa(cc_data.headcount_full_time)} {t("company.full_time")}
                        </>
                      )}
                      {cc_data?.headcount_full_time != null &&
                        cc_data?.headcount_part_time != null &&
                      ...
                      {cc_data?.headcount_part_time != null && (
                        <>
                          {toFa(cc_data.headcount_part_time)} {t("company.part_time")}
                        </>
                      )}
                    </small>
                  )}
                ...
                <div className="kpi">
                  ...
                  <span>{t("company.activity_branch")}</span>
                </div>
              </div>
            </div>
            ...
            {cloudProducts.length > 0 ? (
              <div className="panel">
                ...
                >
                  {cloudProducts.map((p) => (
                    <ProductCard key={p.id} p={p} cc={cc} companyId={c.id} />
                  ))}
                </div>
              </div>
            ) : (
            ...
            <AttachmentGallery
              atts={attachments.filter((a) => a.kind === "gallery_image" && a.is_active)}
            />
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--gap)" }}>
            <div className="panel">
              <h3>
                <Icon name="phone" size={18} className="pi" /> {t("company.contact_methods")}
              </h3>
              {c.contact.phone && (
                <div className="contact-row">
                  ...
                  <div>
                    <div className="cl">{t("company.phone")}</div>
                    <div className="cv">{c.contact.phone}</div>
                  </div>
                </div>
              )}
              {c.contact.email && (
                <div className="contact-row">
                  ...
                  <div>
                    <div className="cl">{t("company.email")}</div>
                    <div className="cv">{c.contact.email}</div>
                  </div>
                </div>
              )}
              {websiteHref && (
                <div className="contact-row">
                  ...
                  <div>
                    <div className="cl">{t("company.website")}</div>
                    <div className="cv">
                      <a
                        href={websiteHref}
                        target="_blank"
            ...
            {park && (
              <button
                ...
                onClick={() => navigate({ to: "/parks", search: { id: park.id } as any })}
              >
                <h3>
                  <Icon name="building" size={18} className="pi" /> {t("company.host_park")}
                </h3>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: 40 }}>
            <div className="section-head">
              <div>
                <span className="eyebrow">{t("company.same_category")}</span>
                <h2 className="h2" style={{ fontSize: 24 }}>
                  {t("company.related_companies")}
                </h2>
              </div>
            </div>
            <div className="grid-cards">
              {related.map((r) => (
                <CompanyCard key={r.id} c={r} />
              ))}
            </div>
          </div>
        )}
    ...
    >
      {vid ? (
        <video
          src={vid}
          controls
          style={{ width: "100%", aspectRatio: "16/10", background: "#000", objectFit: "cover" }}
        />
      ) : img ? (
        <img
          src={img}
...
export function ProductPage({ id, pid }) {
  ...
  if (!product)
    return (
      <div className="view">
        <div className="shell" style={{ padding: 40 }}>
          <h2 className="h2">{t("product.not_found")}</h2>
          <Link to="/company/$id" params={{ id }} className="btn btn-ghost">
            {t("product.back_to_company")}
          </Link>
        </div>
      </div>
    );
    ...
    <div className="view" data-testid="product-detail-page" style={{ ["--accent" as any]: cc }}>
      <div className="shell" style={{ padding: "20px 16px 48px", maxWidth: 1200 }}>
        ...
        >
          ...
          >
            <Link to="/exhibition" style={{ color: "inherit", textDecoration: "none" }}>
              {t("nav.exhibition")}
            </Link>
          ...
          >
            ...
            >
              {companyLogo ? (
                <img
                  src={companyLogo}
                  alt={pickName(company, i18n.language) || ""}
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontWeight: 800, fontSize: 14, lineHeight: 1.2 }}>
                {pickName(company, i18n.language)}
              </div>
            ...
            >
              {product.link_url && (
                <a
                  className="btn btn-primary"
                  href={
                    product.link_url.startsWith("http")
                      ? product.link_url
                      : "https://" + product.link_url
                  }
                  target="_blank"
                  rel="noopener"
            ...
            {otherProducts.length > 0 && (
              <div className="panel" style={{ padding: 20, borderRadius: 24 }}>
                <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 14 }}>
                  {t("product.other_products", { name: pickName(company, i18n.language) })}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {otherProducts.map((p) => (
...
const LABEL_TF = {
  ...
  bottom: "translate(-50%,190%)",
  left: "translate(-112%,-50%)",
  right: "translate(12%,-50%)",
};
...
export function ParksMap({ selectedId }) {
  ...
  const parkXY = (p) => {
    const pr = provById(PARK_PROVINCE[p.id]);
    return pr ? [pr.cx, pr.cy] : [VW / 2, VH / 2];
  };
  ...
  const links = useMemo(() => {
    ...
    PARKS.forEach((a, i) => {
      ...
      const others = PARKS.map((b, j) => {
        const [bx, by] = parkXY(b);
          ...
          <div className="map-stage map-iran parks-map-stage-full">
            ...
            >
                ...
                {provs.map((pr) => {
                  ...
                  return (
                    <path
                      ...
                      fill={
                        cc
                          ? `color-mix(in oklab, ${cc} 40%, transparent)`
                          : hasPark
                            ? "rgba(31,127,214,0.13)"
                            : "rgba(46,66,108,0.10)"
                      }
                      stroke={cc || "rgba(124,170,235,0.26)"}
                      strokeWidth={isSelProv ? 1.8 : 0.7}
              ...
              {PARKS.map((p) => {
                const [cx, cy] = parkXY(p),
                  col = colorVar(p.color),
                  isSel = p.id === sel;
                const rr = 6 + Math.min(8, p.companies / 55);
                return (
                  <g
                    key={p.id}
                    className={"map-node" + (isSel ? " sel" : "")}
                    onClick={() => setSel(p.id)}
            ...
            {PARKS.map((p) => {
              ...
              return (
                <button
                  key={p.id}
                  className={"map-label lp-" + pos + (isSel ? " sel" : "")}
                  onClick={() => setSel(p.id)}
                  style={{
                    left: `${pctX(cx)}%`,
                    top: `${pctY(cy)}%`,
                    transform: LABEL_TF[pos],
                    borderColor: isSel ? colorVar(p.color) : "var(--stroke)",
  ...
  return (
    <div className="park-dashboard" style={{ "--cc": colorVar(park.color) }}>
      ...
      <div className="park-detail-stats">
        <div>
          <b className="num">{toFa(park.companies)}</b>
          <span>{t("park.companies_stat")}</span>
        </div>
        <div>
          <b className="num">{faNum(park.jobs)}</b>
          <span>{t("park.jobs_stat")}</span>
        </div>
        <div>
          <b className="num">{toFa(park.area)}</b>
          <span>{t("park.area_stat")}</span>
        </div>
      </div>
      ...
      {data?.images && data.images.length > 0 && (
        <div className="pd-section">
          ...
          <div className="pd-gallery">
            {data.images.map((img) => (
              <PdImage key={img.id} path={img.image_url} caption={img.caption} />
            ))}
          </div>
        </div>
      )}
      ...
      {parkCompanies.length > 0 && (
        <div className="pd-section">
          <div className="pd-section-title">
            {t("park.resident_companies", { count: parkCompanies.length })}
          </div>
      ...
      <div className="pd-section">
        ...
        <div className="pd-park-list">
          {PARKS.filter((p) => p.id !== sel).map((p) => (
            <button
              key={p.id}
              className="pd-park-row"
              style={{ "--cc": colorVar(p.color) }}
  ...
  return (
    ...
    >
      ...
      <div className="cat-ico">
        <Icon name={CAT_ICON[c.id]} size={26} />
      </div>
