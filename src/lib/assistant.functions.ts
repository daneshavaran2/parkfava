...
export const askAssistant = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const systemPrompt = [
      ...
      "به همان زبانی که کاربر پیام می‌دهد (فارسی یا انگلیسی) پاسخ بده؛ دوستانه و دقیق باش. برای تاکید روی نام شرکت‌ها/پارک‌ها از **دو ستاره** دور کلمه استفاده کن.",
      "وقتی داده‌های زنده‌ی کافی برای سوال کاربر در دسترسه، پاسخ را کامل و ساختاریافته بده — کوتاه‌گویی یا حذف جزئیات مفید (محصولات، بنیان‌گذاران، پتانسیل صادراتی، تماس، وبسایت و…) کار درستی نیست وقتی همون اطلاعات در داده‌های زیر موجوده. اگر بیش از یک شرکت/پارک مرتبطه، هرکدوم را با یک عنوان کوتاه (اسم پررنگ‌شده) از هم جدا کن و برای هرکدوم چند خط توضیح مجزا بده، نه یک پاراگراف فشرده؛ برای فهرست چند مورد از فهرست/بولت استفاده کن. فقط برای سوالات خیلی ساده (مثل «آدرس فلان شرکت چیه؟») پاسخ کوتاه و مستقیم بمون.",
